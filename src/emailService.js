...
// ── Build Weekly Recap Email ─────────────────────────────────────

function buildWeeklyRecapEmail(analystName, startDate, endDate, earningsItems) {
  // Group items by sector → subsector
  const grouped = {};
  for (const item of earningsItems) {
    const sector = item.sector || "Other";
      ...
      for (const item of items) {
        ...
        itemsHtml += `
          <div style="background: #1e293b; border-radius: 8px; padding: 14px 16px; margin-bottom: 8px; border-left: 3px solid ${docBadgeColor};">
            ...
            <div style="margin-top: 4px; color: #cbd5e1; font-size: 13px;">${item.title}</div>
            <div style="margin-top: 4px; color: #94a3b8; font-size: 12px;">Date: ${item.date}</div>
          </div>
        `;
      }
  ...
  const strStart = new Date(startDate).toLocaleDateString("en-US", formatOpts);
  const strEnd = new Date(endDate).toLocaleDateString("en-US", formatOpts);
...
<body style="margin: 0; padding: 0; background-color: #0f172a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
  <div style="max-width: 640px; margin: 0 auto; padding: 24px 16px;">
    ...
    <div style="text-align: center; padding: 32px 20px; background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); border-radius: 16px 16px 0 0; border: 1px solid #334155; border-bottom: none;">
      ...
      <p style="margin: 8px 0 0; color: #f59e0b; font-size: 14px;">
    ...
    <div style="background: #1e293b; padding: 20px 24px; border-left: 1px solid #334155; border-right: 1px solid #334155;">
      ...
      <p style="margin: 8px 0 0; color: #94a3b8; font-size: 13px;">
        Resumen de las empresas en tu cobertura que reportaron entre el <strong style="color: #cbd5e1;">${strStart}</strong> y el <strong style="color: #cbd5e1;">${strEnd}</strong>:
      </p>
    </div>
    ...
    <!-- Content -->
    <div style="background: #0f172a; padding: 24px; border: 1px solid #334155; border-top: none;">
      ${earningsItems.length > 0 ? sectorsHtml : '<p style="color: #94a3b8; text-align: center; font-size: 14px;">No earnings detected for your companies this week.</p>'}
    </div>
    ...
    <div style="text-align: center; padding: 20px; background: #1e293b; border-radius: 0 0 16px 16px; border: 1px solid #334155; border-top: none;">
      <p style="margin: 0; color: #64748b; font-size: 11px;">
        Laguna Capital — Earnings Reminder System<br>
        This is an automated weekly notification. Powered by FMP API.
      </p>
    </div>
...
async function sendWeeklyRecapEmail(analystEmail, analystName, startDate, endDate, earningsItems) {
  const html = buildWeeklyRecapEmail(analystName, startDate, endDate, earningsItems);
  const companiesCount = new Set(earningsItems.map((e) => e.ticker)).size;

  const subject = `📅 Weekly Earnings Recap — ${companiesCount} ${companiesCount === 1 ? "company" : "companies"} reported`;
  ...
  try {
    ...
    console.log(`[Email] Sent Weekly Recap to ${analystEmail}: ${info.messageId}`);
    return true;
  } catch (err) {
    console.error(`[Email] Failed to send Weekly Recap to ${analystEmail}:`, err.message);
    return false;
  }
}
